не вижу особого смысла использовать bash вместо sh

список параметров(то что он принимает list - неточность документации)
...
onbuild
# вариант образа исп ONBUILD для задержки exe команд до сборки образа, команды обрабатываются как часть FROM для образа-потомка.
# могут упростить начальный этап ознакомления со средой, но усложнить в перспективе
